...
- **`src/services/ws.ts`** — singleton WS client: exponential backoff + jitter reconnect, heartbeat `ping` every `heartbeat_seconds` (from `/api/config/`), event dispatch to stores. Dispatches `connected` with `isReconnect`; resync (`GET /api/sync/?after_id=`) and `resendPending()` run **only on reconnect**, never on first connect.
